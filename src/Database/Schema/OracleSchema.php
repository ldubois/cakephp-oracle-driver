...
WHERE 1=1 " . ($useOwner ? $ownerCondition : '') . $objectCondition . " ORDER BY object, name";
    ...
    public function columnSql(TableSchema $schema, string $name): string
    {
        ...
        if ($this->_driver->useAutoincrement()) {
            ...
            if (array_key_exists($data['type'], $integerTypes) && (array) $schema->getPrimaryKey() === [$name]) {
    ...
    public function truncateTableSql(TableSchema $schema): array
    {
        ...
        if (count($keys) == 1 && $this->_driver->useAutoincrement()) {
            ...
            $result[] = sprintf('ALTER TABLE %s MODIFY(%s GENERATED AS IDENTITY (START WITH 1))', $name, $primaryKey);
        }
        return $result;
    }
